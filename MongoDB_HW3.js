...
db.orders.insertMany([
  ...
  db.orders.find({ customerName: "Mia" })

  db.orders.find().sort({ orderDate: -1 }).limit(5)

  db.orders.find({ totalAmount: { $gte: 100, $lte: 500 } })
  .sort({ totalAmount: 1 })
  .skip(1)
  ...
  db.orders.aggregate([
    { $group: { _id: null, averageTotalAmount: { $avg: "$totalAmount" } } }
  ])

  db.orders.aggregate([
    { $group: { _id: "$status", totalAmountSum: { $sum: "$totalAmount" } } }
  ])
